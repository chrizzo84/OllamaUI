...
- **Why**: it could not be trusted, and the last failure said it plainly. Asked for the weather, the assistant answered with a forecast that also advised checking the humidity around an unRAID system, noted that light rain should be fine for a GTX 1060, and reassured the reader that fog is no problem for an RTX 3060. Nothing in the question was about hardware. The mechanism: the question matched no stored fact, and a last-resort rule then filled the prompt with the most recently used ones instead of sending nothing — so the model received a list of graphics cards and, reasonably enough, used them.
- **That was the visible end of a pattern.** Over the preceding days the same feature had to be repaired for storing near-duplicates of the same fact, for storing nothing at all while reporting that it had read everything, for carrying forty durable facts into a question about one of them, and for treating a failed model call as "read it, found nothing" and marking the conversation as done. Each fix was correct; the next failure came from somewhere else in the same system. A feature that rewrites what the assistant believes about a person has to be more reliable than the thing it is helping with, and this one was not.
- **What is kept**: an export of the stored facts is written before the tables are dropped, but only when someone asks for it — this removal takes no copies of its own. Daily database backups (`data/backups/`) still hold the old tables for as long as they are retained, which is the only way back.
- **What is unaffected**: chats, sessions, search, attachments, scheduled tasks, reminders, the Telegram bridge, evaluations, benchmarks, the model catalogue. Scheduled tasks lose their per-task "Memory" checkbox; nothing else about them changes.
- **Tests** — 486, down from 624. The 142 that went were the memory system's own; four new ones cover opening a database that still has the memory tables, dropping them, and leaving every conversation intact.
