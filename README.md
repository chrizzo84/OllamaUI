...
- 📏 Global default context window — **Settings → Generation** sets the `num_ctx` requested from Ollama, stored server-side so it reaches the web chat, the Telegram bridge and scheduled tasks alike (the per-model pill in the chat header is browser-local and never reached the latter two). Worth raising as soon as an MCP server is connected: a large tool catalog is easily tens of thousands of tokens of definitions, and a window too small to hold it is silently truncated, leaving the model a mangled tool list
...
- 🖥️ Running Models page — live `ollama ps` view with CPU/GPU memory split, context window, auto-unload countdown and one-click unload, plus **per-GPU VRAM** on multi-card machines (Ollama's API only ever reports one total across all cards — see [Per-GPU readout](#per-gpu-readout))
...
| **Web search** tool                       | a SearXNG instance (`SEARXNG_HOST`, or Settings → Tools) | Self-hosted. Its JSON format must be enabled — without a reachable, JSON-speaking instance the tool reports an error naming the reason, rather than silently returning no hits |
...
| `OLLAMA_UI_GPU_PROBE`       | `auto`  | Per-GPU readout on the Running page. `auto` probes only when the active Ollama host is loopback, `always` also for a remote-looking address that is in fact this same machine, `off` disables it |
...
#### Telegram Bridge & Voice Transcription (Docker / Homeserver)
...
**On Homeserver**: edit the container → _Add another Path, Port, Variable_ → type **Variable**, with `TELEGRAM_BOT_TOKEN` etc. as Key and the value as Value — no file involved, same as any other env var on that screen (this is also how `OLLAMA_HOST` gets set on Homeserver).
...
The **Running Models** page shows a CPU/GPU split per model. That split comes
from Ollama's `/api/ps`, which reports exactly two numbers per model: `size`
(total resident footprint) and `size_vram`. **`size_vram` is the sum over every
card the model was placed on** — Ollama adds up one GPU buffer at a time and
keeps a per-device map internally (`vramByDevice` in its `llm/llama_server.go`),
but only the total ever leaves the process. No HTTP route exposes the
breakdown, and `ollama ps` doesn't print it either. So the split is already
correct on a multi-GPU box; what it cannot tell you is _which_ card holds what
— and "one card at 95%, the other idle" looks exactly like an even spread.

For that, the page asks the machine it runs on directly (`/api/gpu`) using
whatever is installed — `nvidia-smi`, `rocm-smi`, or Apple Silicon's unified
memory — and lists every card with its own used/total bar and utilization.
Those numbers are the _whole device_, not just Ollama's share, which is the
useful figure when deciding whether the next model still fits.

Because this reads local hardware, it is only shown when that hardware is
provably the hardware Ollama is using:

| `OLLAMA_UI_GPU_PROBE` | Behaviour                                                                                                                 |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `auto` (default)      | Probes only when the active Ollama host is loopback — i.e. the combined Docker image, or both running on the same machine |
| `always`              | Probes regardless. Use when this UI _does_ run on the Ollama machine but addresses it by LAN IP or `host.docker.internal` |
| `off`                 | Never probes                                                                                                              |

With a remote Ollama host the panel is replaced by a line saying so, rather
than by nothing: a missing panel reads like a broken feature, and "the API only
reports a total" is the actual answer. In Docker, the card is only visible
inside the container if the GPU was passed in (see above) — without that,
`nvidia-smi` isn't there and the panel stays absent.
...
| `web_search` returns "response was not JSON"             | The configured endpoint isn't a SearXNG instance, or its `json` format is off | Check Settings → Tools / `SEARXNG_HOST`. Verify with `curl '<endpoint>/search?q=test&format=json'` — HTML back means wrong service or `formats: [html, json]` missing from `settings.yml`. The tool reports this rather than silently returning no hits |
