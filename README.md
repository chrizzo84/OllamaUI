...
- 🛠️ Tool-calling for capable models — `web_search` via SearXNG, `get_current_date`, `get_weather` via Open-Meteo, `calculator`, `create_reminder`, `create_recurring_task`, plus `remember_fact` (memory, its own settings section). Each individually toggleable under Settings → Tools, all on by default; a tool turned off there stays off everywhere — web chat, Telegram, scheduled tasks
